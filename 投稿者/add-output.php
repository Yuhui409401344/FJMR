            ...
            <div class="content">
                ...
                <div class="mt-3 container-fluid">
                    <?php
                        ...
                        $sql7 = $pdo->query("select title,uploader from newpaper_history where title='$title'");
                        foreach($sql7 as $row){
                            $title_isset = $row['title'];
                            $uploader_isset = $row['uploader'];
                        }
                        ...
                        if( isset($stitle) && $level == '接受' && $uploader == $login){
                        ...
                        }else if( isset($stitle) && $title_isset == $title && $uploader != $login){
                            echo "<script> {window.alert('稿件標題已重複，請更改稿件名稱');location.href='add.php'} </script>";
                        }else if( isset($title_isset) && $title_isset == $title && $uploader_isset = $login){
                            echo "<script> {window.alert('稿件已上傳成功，請勿重複上傳，請等待管理者回覆。');location.href=window.history.back()} </script>";
                        }else{
                            ...
                            foreach($sql5 as $row){
                                ...
                                $content = '親愛的管理者您好，輔仁管理評論目前收到一封新的投稿稿件《'.$title.'》，請您盡速到輔仁管理評論的管理者平台分配稿件，謝謝您！';
